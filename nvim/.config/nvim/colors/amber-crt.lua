...
hl("NeoTreeGitIgnored", { fg = c.dim, italic = true })
hl("NeoTreeGitConflict", { fg = c.rust, bold = true, italic = true })
hl("NeoTreeGitUnstaged", { fg = c.amber_d })
hl("NeoTreeGitStaged", { fg = c.olive })
hl("NeoTreeCursorLine", { bg = c.bg_alt })
hl("NeoTreeTitleBar", { fg = c.bg, bg = c.amber, bold = true })
-- Các nhóm dưới nếu bỏ trống sẽ bị plugin tự sinh: chữ pha mờ 30% về nền
-- (gần như tàng hình, vd. "(1 hidden item)") hoặc xám trung tính lệch tông ấm.
hl("NeoTreeMessage", { fg = c.dim, italic = true })
hl("NeoTreeDimText", { fg = c.dim })
hl("NeoTreeDotfile", { fg = c.dim })
hl("NeoTreeHiddenByName", { fg = c.dim })
hl("NeoTreeWindowsHidden", { fg = c.dim })
hl("NeoTreeFadeText1", { fg = c.dim })
hl("NeoTreeFadeText2", { fg = c.sel })
hl("NeoTreeExpander", { fg = c.sel })
hl("NeoTreeModified", { fg = c.amber })
hl("NeoTreeFilterTerm", { fg = c.amber, bold = true })
